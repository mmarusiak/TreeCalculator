...
int main() {
    ...
    Result<Tree*, Error> result = TreeBuilder::buildTree("/ 10 0");
    ...
    Result<Tree*, Error> badResult = TreeBuilder::buildTree("");
    ...
    Result<Tree*, Error> varResult = TreeBuilder::buildTree("+ x 5");
    ...
    if (varResult.isSuccess()) {
        ...
        bool evalSuccess = false;
        double value = varResult.getValue()->calculateTree(vars, evalSuccess);
